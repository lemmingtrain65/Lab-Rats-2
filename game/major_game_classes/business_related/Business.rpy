init -2 python:
    class Business(renpy.store.object):
        ...
        def research_progress(self, int, focus, skill, production_modifier = 1.0):
            research_amount = ((3*int) + focus + (2*skill) + 10) * (self.team_effectiveness / 100.0) * production_modifier
        ...
        def supply_purchase(self,focus,cha,skill, production_modifier = 1.0, cost_modifier = 1.0):
            max_supply = __builtin__.int(((5*focus) + (3*cha) + (3*skill) + 20) * (self.team_effectiveness / 100.0) * production_modifier)
        ...
        def production_progress(self,focus,int,skill, production_modifier = 1.0):
            ...
            production_amount = __builtin__.int(((3*focus) + int + (2*skill) + 10) * (self.team_effectiveness / 100.0) * production_modifier)
